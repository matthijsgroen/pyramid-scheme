...
different one, which then disagrees about the same file and leaves `yarn lint` reporting errors that a
`--write` has just "fixed".

**Never `git pull` on a feature branch here.** `pull.rebase` is true, so a bare pull replays main's
commits as yours and rewrites the branch you were about to push. Use `git merge origin/main`.

**Prettier reflows markdown tables**, so an exact-match edit against a table row you copied out of a file
silently matches nothing and the tool still reports success. Edit tables by line, and read the result back.
...
| 5   | **Changelog**      | Any player-visible change has an entry in `CHANGELOG.md [Unreleased]` — a one-line bullet, one fact, ~20 words, no rationale. **A whole feature is one entry, not an inventory of it**, and `[Unreleased]` is **edited rather than appended to**: if a bullet already covers what you changed, rewrite it instead of adding another. See [`docs/instructions/changelog.md`](docs/instructions/changelog.md). |
| 6   | **Commit message** | Subject line only — `type: what changed`, ≤72 chars. A body is for a WHY the diff cannot show, max ~3 sentences; anything longer belongs in a design doc. See [`docs/instructions/commit-messages.md`](docs/instructions/commit-messages.md).                                                                                                                                                                |

Steps 1–3 and 6 are always required. Steps 4–5 apply only when the change touches user-facing strings or player-visible behavior.
...
- ❌ Writing a commit message body that narrates the work — subject line only unless the diff leaves a WHY unanswered
...
| [`docs/instructions/commit-messages.md`](docs/instructions/commit-messages.md)             | Writing any commit message or PR description — the default is a subject line and no body                                                                              |
...
| Document                                                                                     | Topic                                                                                                                                                                                                                                                                            |
| -------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`docs/game-design/puzzles/`](docs/game-design/puzzles/)                                     | One doc per puzzle family — rules, technique ladder, generation gates, knobs, theming. Catalogue-wide decisions stay in `PUZZLE_FAMILIES.md`                                                                                                                                     |
...
| [`docs/game-design/world-spec-stability.md`](docs/game-design/world-spec-stability.md)       | **Read before changing a floor spec** — which world-spec settings re-carve a floor and which are free (encounters, traps and loot are free); the two invariants the world sweep enforces                                                                                         |
...
| [`docs/game-design/keys-and-locks-solver.md`](docs/game-design/keys-and-locks-solver.md)     | **Built** — the reachability solver (`reachability.ts`, `constraintResolver.ts`) and its per-currency key placement rules; the node-type collapse landed with it (`RoomType` is portal/fork/encounter, a gate being an encounter family)                                         |
...
| [`docs/instructions/map-rendering.md`](docs/instructions/map-rendering.md)                   | **Read before changing how the site map draws** — the map is HTML layers, why (the measurements), the clip/depth/tap rules, and how to measure a change to it                                                                                                                    |
| [`docs/instructions/prop-pipeline.md`](docs/instructions/prop-pipeline.md)                   | **Read before drawing a map tile** — the laws of the projection, then the five steps from deciding what an object is to importing it                                                                                                                                             |
| [`docs/instructions/repaint-queue.md`](docs/instructions/repaint-queue.md)                   | Every prompt still owed to the image generator, with its attachments and its import line                                                                                                                                                                                         |
| [`docs/instructions/art-tasks.md`](docs/instructions/art-tasks.md)                           | The art ledger — what each remaining gap is waiting on, which a census cannot know                                                                                                                                                                                               |
| [`docs/game-design/tile-art-brief.md`](docs/game-design/tile-art-brief.md)                   | What every tile IS, per rank: the authority on subject, which the pipeline defers to                                                                                                                                                                                             |
