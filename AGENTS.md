...
| [`docs/mods/app-plugins-design.md`](docs/mods/app-plugins-design.md)                         | **Design (not yet built)** — app-side mod-plugin seams for a *clean cut*: screen/HUD/reward-effect registries + a per-mod `app.tsx` entrypoint, so core UI names/imports no mod. Closes the `isModEnabled`-in-core + mod-import leaks |
| [`docs/mods/TARGET.md`](docs/mods/TARGET.md)                                                 | **Canonical plan** — mod restructure target state (mod-agnostic core + mod containers + toggle-off gate + DSL-authoring tenet). Per-slice steps in `docs/mods/SLICE-CHECKLIST.md`; archived brainstorm in `docs/mods/_brainstorm.md` |
