...
**Modelling is no longer the bottleneck anywhere — generation is.** The queue is 66 entries, which is
...
`yarn art-census` is the authority and reports 49 placeholders: wizard 20, master 17, expert 12.
...
- **[repaint-queue.md](repaint-queue.md)** — **start here.** Every prompt still owed, with the two images to
  attach and the import line to run afterwards. 66 entries: the priest, the pharaoh, the gods, the shared scatter and conditions, and sixteen patron variants. `yarn repaint <key>`
...
```sh
yarn repaint                  # the 66 keys still owed
...
1. **Work the queue** — 66 entries and every one is a paste rather than a modelling job. `yarn repaint`
...
3. **Patron art — sixteen entries left, and two of them can be rolled today.** `yarn art-census` grew
   ...
   **Two still owed have a painted generic under them and can be rolled today**, and they are 3 rooms:
   ...
   valuable ones — 82 rooms, the two largest pairings in the world among them.
