...
**The projection has a NAME, and naming it is the whole instruction.** Everything below this paragraph
was derived the hard way — three-line depth rules, stacked-top rules, construction procedures, the
generator's own rule set — and all of it together took nine rolls on one jar rack and six on one statue.
...
isometric rotation, and the 16-bit grid names the convention. Use those four phrases; keep the longer
material below only as the explanation of WHY, for a human reading this.
...
| **pit** ⛔        | · no rank authors a hole any more — see below                                                      | ·                                                                                                  | ·                                                                                                            | ·                                                                                       | ·                                                                                             |
...
**⛔ A HOLE IN THE FLOOR MEANS A WAY DOWN, so `pit` is not authored any more.** A staircase is a way to
the floor below and the player can take it; a pit was scenery they could not, and the two are the same
picture — a dark opening in the paving — told apart only by which of them happens to have a ladder in
it. Seven rooms in the world already stood one beside the other. Rather than paint a distinction the
player has to learn, the hole belongs to the stairwell alone.

`starter/pit.png` and `expert/pit.png` are painted and stay in the repository unused: if stair art
turns out to read differently enough, restoring the kind is three lines in the specs. `prim_pit` keeps
a `--contents=plain` that leaves the ladder and its pole out — which is what a pit WOULD need if it
ever came back, and the render that settled this shows why the tile was weaker for it: what is left is
a dark rectangle with four stones at its lip.
