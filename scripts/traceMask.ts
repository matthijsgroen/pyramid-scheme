...
// Output
// ---------------------------------------------------------------------------
const header = `// AUTO-GENERATED by scripts/traceMask.ts — do not edit manually
...
// Two files, because the polygons are 100× the size of the index and only the renderer needs them:
// the game logic (reveal order, currencies, placement queue) works off ids and steps alone, so it
// stays in the startup bundle while the geometry rides along with the lazily loaded mosaic window.
const index = pieces.map(({ points: _points, ...rest }) => rest)

writeFileSync(
  join(__dirname, "../src/ui/atoms/mosaicPieces.generated.ts"),
  `${header}
...
export const MOSAIC_PIECES: MosaicPieceDef[] = ${JSON.stringify(index, null, 2)}
`
)

writeFileSync(
  join(__dirname, "../src/ui/atoms/mosaicGeometry.generated.ts"),
  `${header}
/** Polygon points per piece id, in the SVG \`points\` attribute format. */
export const MOSAIC_POINTS: Record<string, string> = ${JSON.stringify(
    Object.fromEntries(pieces.map(p => [p.id, p.points])),
    null,
    2
  )}
`
...
console.error("Written → src/ui/atoms/mosaicPieces.generated.ts + mosaicGeometry.generated.ts")
