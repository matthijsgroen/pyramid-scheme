 ...
 *   --glow=light.png lay a rendered layer over the art and ADD it, rather than cover with it — which is
 *                    what light does. It is how the exit's shaft can stand where the marker stands: sharp
 *                    has no depth buffer, so a layer composited `over` must be wholly in front of the
 *                    paint or wholly behind it, but an added one simply brightens what it crosses. It
 *                    clips where it crosses something already pale, so render the layer DIM.
 ...
 */
const underlayShadow = async (img: sharp.Sharp, layerPath: string, opacity: number): Promise<sharp.Sharp> => {
  const art = await img.ensureAlpha().png().toBuffer({ resolveWithObject: true })
  const { width, height } = art.info
...
/**
 * Lays a rendered layer over the art and ADDS it, rather than covering with it.
 *
 * Which is what light does, and it is the reason the exit's shaft can exist at all. Composited `over`,
 * every rendered part has to be wholly in front of the paint or wholly behind it — sharp has no depth
 * buffer — so a beam standing where the marker stands could only ever be hidden by it. Added, it brightens
 * whatever it crosses instead: the stone inside the shaft is lit stone, and no depth is needed to say so.
 *
 * `add` CLIPS, and that is the thing to watch rather than the thing to avoid. Where the beam crosses
 * something already pale the sum saturates and detail goes with it, so the layer is rendered dim and the
 * brightness comes from the sum. On the gods' lit calcite that margin is thinnest.
 *
 * The alpha sums as well as the colour, and that is right rather than a side effect: `add` is Porter-Duff
 * PLUS on premultiplied pixels, so where the beam falls outside the stone it keeps its own thin alpha and
 * the paving still shows through it. Capping the result to the art's alpha instead would erase every part
 * of the shaft that is not standing on the pillar, which is most of it.
 */
const glowOver = async (img: sharp.Sharp, layerPath: string): Promise<sharp.Sharp> => {
  const art = await img.ensureAlpha().png().toBuffer({ resolveWithObject: true })
  const { width, height } = art.info
  // Rendered out first for the same reason cutToMask does it: sharp resizes before it composites.
  const fitted = await sharp(layerPath).ensureAlpha().resize(width, height, { fit: "fill" }).png().toBuffer()
  return sharp(
    await sharp(art.data)
      .composite([{ input: fitted, blend: "add" }])
      .png()
      .toBuffer()
  )
}
