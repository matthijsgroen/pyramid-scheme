...
### Changed
- Ward gates that unlock a new difficulty tier no longer all open at once off a single key. Each of a tier's 4 journeys now has its own tier-unlock treasure, so finding more of them opens more of that tier's ward-gated bonus content instead of one key instantly unlocking everything. A few of these treasures now also gate a bonus pocket much later in the game, so a key you found long ago may still have something new to open.
