...
const openChestAndAdvance = (container: HTMLElement) => {
  const chest = container.querySelector('[role="button"]')!
  fireEvent.click(chest)
  // Each step is its own act() so React flushes effects (and registers the next timer)
  // before the fake clock advances again: scheduleLoot's 600ms → LootPopup's own 300ms
  // burst→reveal transition, which is when itemName content actually mounts.
  act(() => vi.advanceTimersByTime(600))
  act(() => vi.advanceTimersByTime(300))
  act(() => vi.advanceTimersByTime(500))
}

describe("ChestRewardFlow — money and sellable rewards", () => {
