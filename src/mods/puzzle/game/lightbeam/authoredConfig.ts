...
import type { AuthoredOptions } from "./generateAuthoredLightbeam"

// The authored generator's tier table (design doc §11.19). It sits beside `LIGHTBEAM_CONFIG` rather than
// replacing it: no tier draws these yet, and the current generator keeps building every board that ships
// until this one has beaten it tier by tier.
//
// **§6.4's vocabulary ladder still sets the shape** — each tier adds one word, rather than every dial moving
// a little — but two of the words have changed hands, because the construction has. What used to be "the goal
// pool" is now `modePool` (§11.18, and the plan's settled decision 1), and what used to be "shadows and
// decoys scattered on the board" is now `branchDepth`: a branch that turns needs a mirror, that mirror is off
// the winning beam's line by construction, and a piece standing in a wrong ray is exactly what §6.1 measured
// as the only thing that makes the technique cap bite.
//
...
export const AUTHORED_LIGHTBEAM_CONFIG: Record<Difficulty, { size: number } & AuthoredOptions> = {
  ...
  // One addition: a longer route, and **stone to die in**. §6.4 asks for walls here and the shipped generator
  // cannot deliver them — measured at 0.00 fixed walls a junior board, because `blockWrongSettings` only adds
  // stone where a wrong ray would otherwise rejoin the route and on a route this length it mostly runs off the
  // frame instead. Wall-heavy is that same intention as a construction rather than a hope: stone closes the
  // branch even where the frame would have done it for nothing.
  junior: {
    size: 8,
    turns: 4,
    interactive: 1,
