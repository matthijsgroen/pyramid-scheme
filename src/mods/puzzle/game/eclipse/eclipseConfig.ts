...
//
// `variants` is the other axis (same doc, "Variants"): the same tier, a different board to read. A tier lists
// `copyFree` only once its cap reaches `noCopy`, because below that the rung is off the ladder already and the
// variant would be `mixed` under another name.
const SIGNED_OR_NOT: EclipseOptions["variants"] = ["mixed", "signless"]
const ALL_VARIANTS: EclipseOptions["variants"] = ["mixed", "signless", "copyFree"]
...
export const ECLIPSE_CONFIG: Record<Difficulty, EclipseOptions> = {
  ...
  starter: { size: 4, techniqueCap: "noTriple", variants: SIGNED_OR_NOT },
  // Counting arrives: a line with two suns in it has two moons coming.
  junior: { size: 4, techniqueCap: "lineCount", requires: ["lineCount"], variants: SIGNED_OR_NOT },
  ...
  // A variant keeps whichever of the two it did not take off the ladder, so the quota still has a rung.
  expert: {
    size: 6,
    techniqueCap: "linePairing",
    requires: ["noCopy", "linePairing"],
    requiresCount: 2,
  ...
  master: {
    size: 6,
    techniqueCap: "squeeze",
    requires: ["loneMark", "squeeze"],
    requiresCount: 3,
    variants: ALL_VARIANTS,
  },
  ...
  wizard: { size: 8, techniqueCap: "squeeze", requires: ["squeeze"], requiresCount: 4, variants: ALL_VARIANTS },
}
