...
export const RushHourBoard: FC<Props> = ({ puzzle, state, skin, hintPiece, hintCells, leaving, onSlide }) => {
  ...
  return (
    <div
      ref={frame}
      className={clsx(
        "relative aspect-square w-full max-w-[min(92vw,60vh)] touch-none overflow-hidden select-none",
        skin.ground
      )}
    ...
    >
      ...
      {/* The way out: a gap in the east wall, on the player's own lane. */}
      <div
        className={clsx("pointer-events-none absolute", skin.exit)}
        style={{ right: 0, top: `${lane * share}%`, height: `${share}%`, width: "3%" }}
      />
