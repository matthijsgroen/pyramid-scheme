...
// The puzzle mod descriptor. Owns the general math-puzzle families (sumplete, balance-scale) and the
// crocodile capstone. A root mod: it stays on in production (turning it off leaves puzzle/capstone
// rooms with no family, so they only auto-resolve via the family-absence pass-through — a degenerate
// world, not a playable one). It is a real REGISTERED_MODS entry anyway so its family metadata flows
// through MOD_FAMILY_META like every other mod's — adding a new puzzle family is then a pure plugin,
// with no edit to core's family list. Toggle-off is the isolation TEST (no core residue), not a
// shipping mode.
//
...
export const puzzleMod: ModDescriptor = {
  id: "puzzle",
  families: [SUMPLETE_META, BALANCE_META, CROCODILE_META],
}
